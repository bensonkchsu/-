...
root = tree.getroot()#建立樹結構圖
#如何把這個樹狀結構轉換成python可以使用的資料?
# 创建一个空字典:在python裡類似數結構的東西，都是用{}做建立

data_dict = {}#data = {key:value}
for element in root:
    key = element.tag  # 讀取tag:x,s,y
    value = element.text  # 讀取裡面的文字10000,10,20(被tag包夾文字)
    data_dict[key] = value#把資料存到python裡面的字典處理
# 打印字典
#print(data_dict)
...
print(compound_interest(x,s,y))#算出解果
#不但要計算出解果且要存至記事本
#打開記事本
